...
import { seedComprehensiveData } from './seed-comprehensive';
...
async function main() {
  ...
  if (existingCount > 0) {
    console.log(`✅ ${existingCount} categories already exist. Skipping category seed.`);
  } else {
    let totalSkills = 0;
    let totalSynonyms = 0;
  ...
  // Run comprehensive seed (users, projects, proposals, etc.)
